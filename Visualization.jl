# ============================================================
#  E-VRP Visualization with CairoMakie
#  98 Customers · 5 Vehicles · 5 Charging Stations · 1 Depot
# ============================================================

using CairoMakie
using Random
using Statistics

# ─────────────────────────────────────────────────────────────
# 1.  PROBLEM DATA
#     Replace the arrays below with your real coordinates.
# ─────────────────────────────────────────────────────────────
Random.seed!(42)

N_CUSTOMERS  = 98
N_VEHICLES   = 5
N_CHARGERS   = 5

# # Depot location
# depot = (50.0, 50.0)
depot= (x=18.190, y=6.320)
# # Customer locations  ← replace with your real data
# customer_x = rand(5.0:0.1:95.0, N_CUSTOMERS)
# customer_y = rand(5.0:0.1:95.0, N_CUSTOMERS)

customer_x= custumers.x
customer_y= custumers.y

# Charger indices (1-based, into the customer array)
# ← replace with the indices of the 5 customers that have chargers
charger_indices = [12, 27, 45, 63, 81]

# ─────────────────────────────────────────────────────────────
# 2.  SIMPLE ROUTE ASSIGNMENT
#     Nearest-neighbour greedy assignment – one pass per vehicle.
#     Replace `routes` with your optimised solution if available.
# ─────────────────────────────────────────────────────────────

function nearest_neighbour_routes(cx, cy, depot, n_vehicles)
    n = length(cx)
    assigned = falses(n)
    routes   = [Int[] for _ in 1:n_vehicles]

    for v in 1:n_vehicles
        current = depot          # start at depot
        while true
            # find closest unassigned customer
            best_d = Inf
            best_i = -1
            for i in 1:n
                assigned[i] && continue
                d = hypot(cx[i] - current[1], cy[i] - current[2])
                if d < best_d
                    best_d = d
                    best_i = i
                end
            end
            best_i == -1 && break   # no customers left
            push!(routes[v], best_i)
            assigned[best_i] = true
            current = (cx[best_i], cy[best_i])

            # limit per vehicle so all 5 share the load roughly equally
            remaining = count(!, assigned)
            vehicles_left = n_vehicles - v + 1
            if length(routes[v]) >= ceil(Int, (n - sum(length.(routes[1:v-1]))) / vehicles_left)
                break
            end
        end
    end
    return routes
end

routes = nearest_neighbour_routes(customer_x, customer_y, depot, N_VEHICLES)

# Summarise
println("Route sizes: ", length.(routes))

# ─────────────────────────────────────────────────────────────
# 3.  COLOUR PALETTE  (LIST-inspired)
# ─────────────────────────────────────────────────────────────
vehicle_colors = [
    colorant"#FF3CAC",   # hot pink / magenta
    colorant"#F9C74F",   # vivid yellow
    colorant"#43E8A0",   # neon mint-green
    colorant"#4CC9F0",   # electric sky-blue
    colorant"#FF6B35",   # bright orange
]

# ─────────────────────────────────────────────────────────────
# 4.  FIGURE SETUP
scale_factor = 1  # for HiDPI export; set to 1 for normal resolution
font_factor = 1.2 * scale_factor
# ─────────────────────────────────────────────────────────────
fig = Figure(size = (1200, 900).*scale_factor, backgroundcolor = :white)

ax = Axis(
    fig[1, 1],
    title       = "Electric Vehicle Routing Problem  —  5 Vehicles · 98 Customers · 5 Chargers",
    titlesize   = 18*font_factor,
    titlecolor  = RGBf(83/255, 121/255, 146/255),
    titlefont   = :bold,
    xlabel      = "X Coordinate",
    ylabel      = "Y Coordinate",
    xlabelsize  = 13*font_factor,
    ylabelsize  = 13*font_factor,
    xgridcolor  = RGBAf(0.85, 0.85, 0.85, 0.6),
    ygridcolor  = RGBAf(0.85, 0.85, 0.85, 0.6),
    # backgroundcolor = RGBf(0.07, 0.07, 0.12)   # deep navy-black,
)

# ─────────────────────────────────────────────────────────────
# 5.  DRAW ROUTES  (depot → customers → depot)
# ─────────────────────────────────────────────────────────────
for (v, route) in enumerate(routes)
    isempty(route) && continue
    col = vehicle_colors[v]

    # Build full path: depot → route → depot
    xs = [depot[1]; customer_x[route]; depot[1]]
    ys = [depot[2]; customer_y[route]; depot[2]]

    lines!(ax, xs, ys;
        color     = (col, 0.55),
        linewidth = 1.8,
        linestyle = :solid,
    )

    # Arrows on each segment to show direction
    for i in 1:length(xs)-1
        dx = xs[i+1] - xs[i]
        dy = ys[i+1] - ys[i]
        mid_x = xs[i] + 0.5dx
        mid_y = ys[i] + 0.5dy
        arrows!(ax, [mid_x], [mid_y], [dx*0.12], [dy*0.12];
            color      = (col, 0.8),
            arrowsize  = 8,
            linewidth  = 0,
        )
    end
end

# ─────────────────────────────────────────────────────────────
# 6.  DRAW CUSTOMERS
# ─────────────────────────────────────────────────────────────
# colour each dot by the vehicle that serves it
dot_colors = Vector{Any}(undef, N_CUSTOMERS)
for (v, route) in enumerate(routes)
    for i in route
        dot_colors[i] = vehicle_colors[v]
    end
end

scatter!(ax, customer_x, customer_y;
    color      = dot_colors,
    marker     = :circle,
    markersize = 14*font_factor,
    strokewidth = 0.8,
    strokecolor = :white,
)

# Customer index labels (small, subtle)
for i in 1:N_CUSTOMERS
    text!(ax, customer_x[i] , customer_y[i];
        text     = string(i),
        fontsize = 13*font_factor,
        color    = RGBAf(0.3, 0.3, 0.3, 0.8),
        align    = (:left, :bottom),
    )
end

# ─────────────────────────────────────────────────────────────
# 7.  DRAW CHARGING STATIONS
# ─────────────────────────────────────────────────────────────
for idx in charger_indices
    # Glowing halo
    scatter!(ax, [customer_x[idx]], [customer_y[idx]];
        color      = (colorant"#FFE566", 0.3),
        marker     = :circle,
        markersize = 40,
    )
    # Charger symbol (⚡ as text marker)
    scatter!(ax, [customer_x[idx]], [customer_y[idx]];
        color      = colorant"#FFE566",
        font= "Bold",
        marker     = '⚡',
        markersize = 26,
    )
end

# ─────────────────────────────────────────────────────────────
# 8.  DRAW DEPOT
# ─────────────────────────────────────────────────────────────
# Outer ring
scatter!(ax, [depot[1]], [depot[2]];
    color      = (colorant"#000000", 0.15),
    marker     = :circle,
    markersize = 30,
)
# Filled star
scatter!(ax, [depot[1]], [depot[2]];
    color      = colorant"#000000",
    marker     = '★',
    markersize = 22,
)
text!(ax, depot[1], depot[2];
    text     = "Depot",
    fontsize = 11*font_factor,
    color    = :black,
    font     = :bold,
    align    = (:center, :top),
)

# ─────────────────────────────────────────────────────────────
# 9.  LEGEND
# ─────────────────────────────────────────────────────────────
legend_elements = [
    # One entry per vehicle
    [LineElement(color = vehicle_colors[v], linewidth = 2.5),
     MarkerElement(color = vehicle_colors[v], marker = :circle, markersize = 10)]
    for v in 1:N_VEHICLES
]

legend_labels = ["Vehicle $v  ($(length(routes[v])) stops)" for v in 1:N_VEHICLES]

# Depot & charger entries
push!(legend_elements, [MarkerElement(color = :black, marker = '★', markersize = 14)])
push!(legend_labels,   "Depot")
push!(legend_elements, [MarkerElement(color = colorant"#00AEC8", marker = '⚡', markersize = 14)])
push!(legend_labels,   "Charging Station (×$(N_CHARGERS))")

Legend(
    fig[1, 2],
    legend_elements,
    legend_labels;
    framecolor  = RGBf(83/255, 121/255, 146/255),
    framewidth  = 1.2,
    labelsize   = 12*font_factor,
    titlesize   = 13*font_factor,
    title       = "Legend",
    titlefont   = :bold,
    titlecolor  = RGBf(83/255, 121/255, 146/255),
    padding     = (12, 12, 12, 12),
    rowgap      = 6,
    patchsize   = (25, 18),
)

# ─────────────────────────────────────────────────────────────
# 10.  STATS PANEL (right column, below legend)
# ─────────────────────────────────────────────────────────────
function route_distance(route, cx, cy, depot)
    isempty(route) && return 0.0
    d  = hypot(cx[route[1]] - depot[1], cy[route[1]] - depot[2])
    for i in 2:length(route)
        d += hypot(cx[route[i]] - cx[route[i-1]], cy[route[i]] - cy[route[i-1]])
    end
    d += hypot(depot[1] - cx[route[end]], depot[2] - cy[route[end]])
    return d
end

dists = [route_distance(routes[v], customer_x, customer_y, depot) for v in 1:N_VEHICLES]

stats_text = join([
    "── Route Statistics ──",
    "",
    (["Veh $v:  $(length(routes[v])) stops,  dist=$(round(dists[v]; digits=1))"
      for v in 1:N_VEHICLES])...,
    "",
    "Total distance : $(round(sum(dists); digits=1))",
    "Avg / vehicle  : $(round(mean(dists); digits=1))",
    "Charger nodes  : $(N_CHARGERS)",
], "\n")

# Label(
#     fig[2, 2],
#     stats_text;
#     font       = :regular,
#     fontsize   = 11*font_factor,
#     color      = RGBf(0.1, 0.1, 0.1),
#     halign     = :left,
#     valign     = :top,
#     padding    = (12, 12, 12, 12),
#     tellheight = false,
# )


# ─────────────────────────────────────────────────────────────
# 12.  LAYOUT TUNING & SAVE
# ─────────────────────────────────────────────────────────────
colsize!(fig.layout, 1, Relative(0.78))   # map takes 78 % of width
rowsize!(fig.layout, 1, Relative(0.93))

save("evrp_visualization.png", fig; px_per_unit = 2)  # 2× for HiDPI
display(fig)

println("\n✓ Figure saved as  evrp_visualization.png")